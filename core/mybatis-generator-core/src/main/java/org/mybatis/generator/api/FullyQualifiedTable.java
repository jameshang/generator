...
import org.mybatis.generator.plugins.DomainObjectNameProcessor;
...
public class FullyQualifiedTable {
	...
	private String domainObjectNameProcessor;
	 ...
	 *            the actual catalog of the table as returned from DatabaseMetaData. This value should only be set if the user configured a catalog. Otherwise the DatabaseMetaData
	 *            is reporting some database default that we don't want in the generated code.
	 * @param introspectedSchema
	 *            the actual schema of the table as returned from DatabaseMetaData. This value should only be set if the user configured a schema. Otherwise the DatabaseMetaData is
	 ...
	 *            the configured domain object name for this table. If nothing is configured, we'll build the domain object named based on the tableName or runtimeTableName.
	 * @param alias
	 *            a configured alias for the table. This alias will be added to the table name in the SQL
	 * @param ignoreQualifiersAtRuntime
	 *            if true, then the catalog and schema qualifiers will be ignored when composing fully qualified names in the generated SQL. This is used, for example, when the
	 ...
	 *            this is used to "rename" the catalog in the generated SQL. This is useful, for example, when generating code against one catalog that should run with a different
	 *            catalog.
	 * @param runtimeSchema
	 *            this is used to "rename" the schema in the generated SQL. This is useful, for example, when generating code against one schema that should run with a different
	 *            schema.
	 * @param runtimeTableName
	 *            this is used to "rename" the table in the generated SQL. This is useful, for example, when generating code to run with an Oracle synonym. The user would have to
	 ...
	 *            if true, then the table identifiers will be delimited at runtime. The delimiter characters are obtained from the Context.
	 * @param context
	 *            the context
	 */
	...
	public String getDomainObjectName() {
		String don = null;
		if (stringHasValue(domainObjectName)) {
			don = domainObjectName;
		} else if (stringHasValue(runtimeTableName)) {
			don = getCamelCaseString(runtimeTableName, true);
		} else {
			don = getCamelCaseString(introspectedTableName, true);
		}
		return this.processDomainObjectName(don);
	}

	private String processDomainObjectName(String don) {
		if (!stringHasValue(don)) {
			return "";
		}
		if (!stringHasValue(this.domainObjectNameProcessor)) {
			return don;
		}
		try {
			Class<?> clazz = Class.forName(this.domainObjectNameProcessor.trim());
			if (DomainObjectNameProcessor.class.isAssignableFrom(clazz)) {
				DomainObjectNameProcessor processor = (DomainObjectNameProcessor) clazz.newInstance();
				return processor.process(don);
			}
		} catch (Exception e) {
		}
		return don;
	 ...
	 * 
	 * This method is used for determining the sub package for Java client and SQL map (XML) objects. It ignores any sub-package added to the domain object name in the table
	 * configuration.
	 *
	 ...
	 * Calculates a Java package fragment based on the table catalog and schema. If qualifiers are ignored, then this method will return an empty string.
	 * 
	 * This method is used for determining the sub package for Java model objects only. It takes into account the possibility that a sub-package was added to the domain object name
	...
	public String getDomainObjectNameProcessor() {
		return domainObjectNameProcessor;
	}

	public void setDomainObjectNameProcessor(String domainObjectNameProcessor) {
		this.domainObjectNameProcessor = domainObjectNameProcessor;
	}
